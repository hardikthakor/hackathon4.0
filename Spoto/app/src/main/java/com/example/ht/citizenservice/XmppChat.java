...
public class XmppChat extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.i("------final", AJabberId);

        //final String[] jaber = AJabberId.split("\\@");
        //yourName = jaber[0];

        sharedpreferences = getSharedPreferences(pref, Context.MODE_PRIVATE);
        jabberId = sharedpreferences.getString("jabberId", null);

        Log.i("------final", jabberId);

        yourName = AJabberId;
        myName = jabberId;

        icon1 = BitmapFactory.decodeResource(getResources(), R.drawable.userprofile_ic_person_black_24dp);
        icon2 = BitmapFactory.decodeResource(getResources(), R.drawable.userprofile_ic_person_black_24dp);

        me = new User(myId, myName, icon1);
        you = new User(yourId, yourName, icon2);
